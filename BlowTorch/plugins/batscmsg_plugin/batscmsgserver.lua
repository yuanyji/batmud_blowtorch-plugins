...
DirectionConstant1 = {"n", "w", "s", "e", "nw", "ne", "sw", "se", "up", "down", "enter", "out"}
DirectionConstant2 = {"s", "e", "n", "w", "se", "sw", "ne", "nw", "down", "up", "out", "enter"}
...
function initializationAreaMapDraw()
    ...
    row = 31
    index = row / 2 * column
    columnstr = "|"..string.rep(" ", column-3).."|\n"
    areamap = ","..string.rep("-", column-3)..",\n"..string.rep(columnstr, row - 2).."'"..string.rep("-", column-3).."'\n"
...
function ifDrawMap(val)
  ...
  elseif nextPosition == "ed" then
    drawmapmode = "false"
    EnableTrigger("roominfo",false)
    initializationAreaMapDraw()
    SendToServer("brief brief")
    areaname = nil
  elseif nextPosition == "sv" then
...
--  if stringmap["1"]:find("Obvious exit") then
 ...
 -- end
  ...
  if conjectInfo then
  roomdescription1 = roomdescription1.." "..stringmap["1"]
    ...
    for _, data in ipairs(roomInfo) do
        if data.Info == roomdescription then
           -- WindowXCallS("batscmsgwindow", "ifStartDrawAreaMap", "cancel")
            ...
            --Note("\n".."yiwen: "..exitTable[existenceroomorder].."←→".."-"..counternextPosition.."-")
...
if nextPosition ~= "" and roominformation == "true" then
  ...
  --  currentroomindex = index
    ...
    findRangOfCoords()
    --BackUpMap3 = BackUpMap2
    --BackUpMap2 = BackUpMap1
    --BackUpMap1 = BackUpMap0
    --BackUpMap0 = BackUpMapDraw()
    --roominformation = "false"
...
function translationContent()
   ...
   local runner = {}
   runner.run = function()
      --local searchUrl = luajava.newInstance("java.lang.String", "http://127.0.0.1:9000/translate")
      local searchUrl = luajava.newInstance("java.lang.String", "https://deeplx.gobob.top/translate")
     -- local searchUrl = luajava.newInstance("java.lang.String", "https://deeplx.gobob.top/V1/translate")
    --  local searchUrl = luajava.newInstance("java.lang.String", "https://deeplx.gobob.top/V2/translate")
      ...
      if responseCode == 200 then
            ...
            reader:close()
            local endIndex = string.find(response,"\"id\"") - 3
            local outputText = string.sub(response, 21, endIndex):gsub("\\", "")
            if source_lang == "zh" then
              ...
              --  local pattern = "   "
              -- local segments = {}
              --for segment in outputText:gmatch("([^" .. pattern .. "]+)") do
              --      table.insert(segments, segment)
               -- end

               -- 遍历所有片段，传递给process函数处理
              -- for _, segment in ipairs(segments) do
               --     processTranslationResult(segment)
              --  end
            end
      else
...
function targetnpc(name, raw, map)
  ...
  -- Note(str6.."\n")
   ...
   local NPC = {}
   local npcexit = "no"
   local npcfile = io.open("/storage/emulated/0/BlowTorch/logfiles/Batmud_npcLog.txt", "r")
...
function processMonsterInfoResult(Data)
    ...
    local areaname1 = areaname or NewPosition
    npclog	= io.open("/storage/emulated/0/BlowTorch/logfiles/Batmud_npcLog.txt", "a+")
    npclog:write(",\r{"..NewContinent.."_"..areaname1.."\r"..result2.."}")
    npclog:flush()
    npclog:close()
    if NewRoomMessage[NewContinent][areaname][ManualAllOd] then
      local roommonsters = NewRoomMessage[NewContinent][areaname][ManualAllOd]
      if not roommonsters["Monster"] then
      roommonsters["Monster"] = {}
      end
      table.insert(NewRoomMessage[NewContinent][areaname][ManualAllOd]["Monster"], "{"..result2.."}")
    end
    else
